...
def dataSetLookup(number):
    database = loadDatabase()
    listOfCodes = []
    for code in database.keys():
        if 'dataSetNumber' in database[code].keys():
            for dbnumber in database[code]['dataSetNumber']:
                if dbnumber == number:
...
def getInfo(code):
    ...
    if code in database.keys():
        if 'swagerUser' in database[code].keys():
        	swageLength = database[code]['swageLength']
        	rawLength = database[code]['rawLength']
        	swageDate = database[code]['swagerDate']
        	cCode = database[code]['cCode']
        	eCode = database[code]['eCode']
        	swagerComment = database[code]['swagerComment']
        	return [rawLength[0],swageLength[0]],swageDate[0], cCode[0], eCode[0], swagerComment[0]
...
def write(code, lengths, cleanCode, errorCode, name, endplugcode):
    try:
        ...
        with open(filename, 'r') as ff:
            ...
            lines.append(f"{code},{lengths[0]},{lengths[1]},\
{datetime.now().strftime('%m.%d.%Y_%H_%M_%S')},{cleanCode},\
{errorCode},{comment},{name},{endplugcode}\n")
            open(filename,'w').writelines(lines)
        
    except FileNotFoundError:
        open(filename,'w').close()
        write(code, lengths, cleanCode, errorCode, name, endplugcode)
    ...
    if code in database.keys():
        ...
        if 'tensionUser' in database[code].keys():
            for number, user in enumerate(database[code]['tensionUser']):
                printString+=str(database[code]['tensions'][number]) \
                + '\t\t' + str(user) + '\t\t' \
        ...
        if 'leakUser' in database[code].keys():
            for number, leakRate in enumerate(database[code]['leakRate']):
                printString+= database[code]['leakUser'][number] + \
                               '\t' + str(leakRate) + '\n'
        else:printString += 'None\n'
        ...
        lengths, date, cleanCode, errorCode, comment = getInfo(code)
        if cleanCode == []:
            printString+="No codes or lengths availible\n"
        else:
            printString+="Raw Length: " + lengths[0] + '\n'
            printString+="Swaged Length: " + lengths[1] + '\n'
            printString+="Cleaning Code " + cleanCode + '\n'
            printString+="Error Code " + errorCode + '\n'
            printString+="Comment: " + comment + '\n'
        
        printString+='\n\n----------Files in database for this tube----------\n'
        printString+='\nTENSION FILES\n'
        if 'tensionUser' in database[code].keys():
