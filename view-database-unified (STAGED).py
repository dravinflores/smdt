...
# Note that this is the Linux copy.
def OnSearching(sv):
    code = sv.get()
    ...
    if code == '':
        ...
        if code in database.keys():
            ...
            if 'swagerUser' in database[code].keys():
                ...
                if 'failsSwager' in database[code].keys() and \
                        database[code]['failsSwager']==True:
                ...
                if 'failsFirstTension' in database[code].keys() and \
                        database[code]['failsFirstTension'] == True:
                    printString+='Tube failed first tension\n'
                elif 'failsLeak' in database[code].keys() and \
                        database[code]['failsLeak']==True:
                    printString+='Tube failed leak test\n'
                elif 'failsCurrent' in database[code].keys() and \
                        database[code]['failsCurrent']==True:
                    printString+='Tube failed dark current test\n'
                elif 'failsSecondTension' in database[code].keys() and \
            ...
            if 'tensionUser' in database[code].keys():
                ...
                for number, firstTension in \
                        enumerate(database[code]['firstTensions']):
                ...
                for number, secondTension in \
                        enumerate(database[code]['secondTensions']):
                    printString += \
                        database[code]['secondTensionUsers'][number] \
                        + ': ' + str(secondTension) + '\t\t' \
                ...
                tensionText.insert('1.0','No Information Available\n')
            
            # Dark Current
            if 'currentFile' in database[code].keys():
                printString='Dark Current' + '\t\t' + 'Test Date\n' 
                for number, current in \
                        enumerate(database[code]['darkCurrents']):
                    printString += current + '\t\t' \
                        + database[code]['currentTestDates'][number]
...
def updatePlots(data): 
    # data is [tension1, tension2, diff tension, leak, current, errors]
    global axes
    global bar1
    tenDF = DataFrame([-1])
    if data[0] != []: 
    ...
    leakDF.plot(kind='hist', 
        bins=np.logspace(np.log10(0.0000000001),np.log10(0.0001), 50), 
...
def updateTreeView(event):
    ...
    for tube, tubeData in sorted(database.items(), reverse=True):
        ...
        if len(dateParts) == 3: 
            swagerDate = '{1}.{0}.{2}'.format(
                    dateParts[0],
                    dateParts[1],
                    dateParts[2]
                )
        else: 
            swagerDate = swagerDate = tubeData.get("swagerDate")[0]
        status = 'uncertain'

        if 'good' in tubeData.keys():
            if tubeData.get('good') == True: 
                status = 'good'
            else: 
                status = 'bad'
        ...
        if 'leakRate' in tubeData.keys() and tubeData.get('leakRate') != []: 
            leakRate = "{0:.2g}".format(min(tubeData.get('leakRate')))
        else: 
            leakRate = "None"

        darkCurrent = "None"
        if 'darkCurrents' in tubeData.keys() and \
                tubeData.get('darkCurrents') != []:
        ...
        if 'swageLength' in tubeData.keys() and \
                tubeData.get('swageLength')[0]!='': 
...
def StringToDateTime(str):
    ...
    while not isError:
        try:
            return_obj = datetime.strptime(str, fmt)
            break
        except:
            isError = True
    
    if isError:
        # Just use a placeholder date.
        return datetime(2000, 1, 1, 0, 0, 0)
    else:
        return return_obj

# This function handles sorting the database by whatever the user clicks.
def SortOnClick(sort_by, rev):
    # Clear the entire board.
    database = sy.synchronize()
    tree.delete(*tree.get_children())

    # Note that sort_by can have the following values: 
    # "Tube ID", "Swage User", "Swage Date", "Tens. 1 Date", "Tens. 2 Date"

    # We decide how to sort the database

    db = sorted(database.items(), reverse=True)

################################################################################
#   Notes on How The Database is Sorted.
#       - The top layer is a list full of tuples. We will call this db.
#       - db[0] is the Barcode
#       - db[1] is the dictionary storing the data.
#       - 
#
#   Notes on the errors encountered.
#       - Some 'swagerUser' lists are empty.
#
################################################################################

    # We need to filter before we sort.

    filtered_db = list()
    
    # The filtering process.
    for (tubeID, tubeData) in db:
        if not tubeID or 'swagerUser' not in tubeData.keys():
            continue
        ...
        if 'darkCurrents' in tubeData.keys() and \
                tubeData.get('darkCurrents') != []:
            darkCurrent =  database[tubeID]['darkCurrents'][-1]
        rawLength = 'None'
        swageLength = 'None'
        ...
        swagerDate = StringToDateTime(swagerDate)
        tension1Date = StringToDateTime(tension1Date)
        tension2Date = StringToDateTime(tension2Date)

        filtered_db.append(
            [barcode, swagerUser, swagerDate, tension1Date, \
            tension1, tension2Date, tension2, leakRate, \
            darkCurrent, rawLength, swageLength, status]
        )
    
    sorted_db = list()

    if sort_by == "Tube ID":
        sorted_db = sorted(filtered_db, key=lambda tube : tube[0], reverse=rev)
    elif sort_by == "Swage User":
        sorted_db = sorted(filtered_db, key=lambda tube : tube[1].casefold(), reverse=rev)
    elif sort_by == "Swage Date":
        sorted_db = sorted(filtered_db, key=lambda tube : tube[2], reverse=rev)
    elif sort_by == "Tens. 1 Date":
        sorted_db = sorted(filtered_db, key=lambda tube : tube[3], reverse=rev)
    elif sort_by == "Tens. 2 Date":
        sorted_db = sorted(filtered_db, key=lambda tube : tube[4], reverse=rev)
    else:
        pass

    """
    with open("db_text.txt", 'w') as db_text:
        for i in filtered_db:
            db_text.write(str(i) + "\n\n")
    """

    noDate = datetime(2000, 1, 1, 0, 0, 0)

    for tube in sorted_db:
        barcode = tube[0]
        swagerUser = tube[1]
        swagerDate = tube[2]
        tension1Date = tube[3]
        ...
        if swagerDate == noDate:
            swagerDate = "No Date"
        else:
            swagerDate = swagerDate.strftime('%m/%d/%y %H:%M:%S')

        if tension1Date == noDate:
            tension1Date = "No Date"
        else:
            tension1Date = tension1Date.strftime('%m/%d/%y %H:%M:%S')

        if tension2Date == noDate:
            tension2Date = "No Date"
        else:
            tension2Date = tension2Date.strftime('%m/%d/%y %H:%M:%S')
    ...
    tree.heading("#0",text="Tube ID",anchor=tk.W, command=lambda:SortOnClick("Tube ID", not rev))
    tree.heading("two", text="Swage User",anchor=tk.W, command=lambda:SortOnClick("Swage User", not rev))
    tree.heading("twopfive", text="Swage Date", anchor=tk.W, command=lambda:SortOnClick("Swage Date", not rev))
    tree.heading("three", text="Tens. 1 Date",anchor=tk.W, command=lambda:SortOnClick("Tens. 1 Date", not rev))
    tree.heading("five", text="Tens. 2 Date",anchor=tk.W, command=lambda:SortOnClick("Tens. 2 Date", not rev))
...
if __name__ == "__main__":
    ...
    tree.heading("#0",text="Tube ID",anchor=tk.W, command=lambda:SortOnClick("Tube ID", True))
    tree.heading("two", text="Swage User",anchor=tk.W, command=lambda:SortOnClick("Swage User", True))
    tree.heading("twopfive", text="Swage Date", anchor=tk.W, command=lambda:SortOnClick("Swage Date", True))
    tree.heading("three", text="Tens. 1 Date",anchor=tk.W, command=lambda:SortOnClick("Tens. 1 Date", True))
    tree.heading("four", text="Tension 1",anchor=tk.W)
    tree.heading("five", text="Tens. 2 Date",anchor=tk.W, command=lambda:SortOnClick("Tens. 2 Date", True))
