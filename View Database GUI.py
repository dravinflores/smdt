...
def OnSearching(sv):
    ...
    if code == '':
        ...
        if code in database.keys():
            ...
            # Dark Current
            if 'finalCurrents' in database[code].keys():
                printString=''
                for number, current in enumerate(database[code]['finalCurrents']):
                    printString+=current + '\t\t' + database[code]['currentTestDate'][number]
                    for tube in database[code]['currentTestGroup'][number]:
                        printString+= '\n\t' + tube
                    printString+='\n'
                printString+='\nBEST TEST:\n'
                if 'testType' in database[code].keys():
                    if database[code]['testType'] == 'single':
                        printString += database[code]['finalCurrent'] + 'nA\t\t' + \
                            "Single Tube Test\t\t" + \
                            str(database[code]['currentTestDate'][number] + '\n')
                    else:
                        printString += database[code]['finalCurrent'] + 'nA\t\t' + \
                            "Muliple Tube Test\t\t" + \
                            str(database[code]['currentTestDate'][number] + '\n')
                printString+='\nDARK CURRENT FILES\n'
                for file in database[code]['currentFile']:
                    printString+=file + '\n'  
...
def updateTreeView(event):
    ...
    for tube, tubeData in sorted(database.items(), reverse=True):
        ...
        darkCurrent = "None"
        if 'finalCurrent' in tubeData.keys():
            if tubeData.get('testType') == 'single':
                darkCurrent = '\t%.1fS\t' % float (database[tube]['finalCurrent'])
            elif tubeData.get('finalCurrent') != []: 
                darkCurrent = '\t%.1fM\t' % float (database[tube]['finalCurrent'])
        rawLength = 'None'
        swageLength = 'None'
        if 'rawLength' in tubeData.keys() and tubeData.get('rawLength')[-1]!='': 
            rawLength = 1624.3 + float(tubeData.get('rawLength')[-1])
            rawLength = '{0:.6g}'.format(rawLength)
        if 'swageLength' in tubeData.keys() and tubeData.get('swageLength')[-1]!='': 
            swageLength = 1624.3 + float(tubeData.get('swageLength')[-1])
...
def getData(minTube, maxTube):
    ...
    for tube,tubeData in database.items():
        if tube > minTube and tube < maxTube:
            ...
            if 'finalCurrent' in tubeData.keys() and tubeData.get('finalCurrent') != []:
                final_currents.append(float(tubeData.get('finalCurrent')))
...
if __name__ == "__main__":
    ...
    swageLabel = tk.Label(master=f2, text="Swage Data").grid(row=1,column=0)
    swageText = tk.Text(master=f2,width=70,height=28)
    swageText.grid(row=2,column=0)
    
    tensionLabel = tk.Label(master=f2, text='Tension Data').grid(row=3,column=0)
    tensionText = tk.Text(master=f2,width=70,height=28)
    tensionText.grid(row=4,column=0)
    
    leakLabel = tk.Label(master=f2, text='Leak Data').grid(row=1,column=1)
    leakText = tk.Text(master=f2,width=70,height=28)
    leakText.grid(row=2,column=1)
    
    darkLabel = tk.Label(master=f2,text='Dark Current Data').grid(row=3,column=1)
