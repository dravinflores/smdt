...
from sMDT.old_legacy import station_pickler
...
class db:
    def __init__(self):
        ...
        self.db_file = self.dropbox_directory / 'old_database.s'
        self.lock_file = self.dropbox_directory /'sMDT'/'locks'/'old_db_lock.lock'
    ...
    def add_tube(self, tube=MSU_only_Tube()):
    ...
    def delete_tube(self, tube_id):
        #unless the remove is in Tube(), only creates .del.tube, doesn't remove from database
    ...
    def update(self, logging=True):
        ...
        with portalocker.Lock(s, 'r+', timeout=30) as locked_file:
            with shelve.open(self.path) as tubes:
                log_activity = open('old_activity.log', 'a')
