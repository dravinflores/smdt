...
for subdir, dirs, files in os.walk(dropbox_dir):
    for filename in files:
        ...
        if any([filepath.endswith(ext) for ext in tracked_extensions]):
...
if matches:
    print("\nComplete Matches:")
    [print(path) for path in sorted(matches, key=lambda x: x.split('.')[-1])]
if mismatches:
    print("\nIncomplete Matches:" + (" - Now Complete matches based on the " + ("GitHub" if mode == 3 else "Dropbox") + " version" if mode in [3,4] else ""))
    [print(path) for path in sorted(mismatches, key=lambda x: x.split('.')[-1])]
if github_only:
    print("\nOnly on GitHub" + (" - Now added to Dropbox" if mode == 2 else ""))
    [print(path) for path in sorted(github_only, key=lambda x: x.split('.')[-1])]
if dropbox_only:
    print("\nOnly on Dropbox" + (" - Now added to GitHub" if mode == 2 else ""))
    [print(path) for path in sorted(dropbox_only, key=lambda x: x.split('.')[-1])]